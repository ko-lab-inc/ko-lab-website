    ...
    <div className="flex min-h-screen items-center justify-center bg-[#0a0f1a] p-6 font-mono text-white">
      ...
      >
        ...
        <input
          ...
          required
          autoFocus
          className="mt-2 w-full rounded-lg border border-white/15 bg-transparent px-4 py-3 text-base text-white outline-none focus:border-cyan-400/60"
        />
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1a] p-5 font-mono text-white">
      <header className="mb-6 flex items-center justify-between">
        ...
        {/* Discret, en coin — pas un gros bouton rouge : un tap accidentel
            avec des gants ne doit pas être facile (brief). */}
        <form action={deconnexionStaff}>
          <button type="submit" className="rounded-md border border-white/10 px-3 py-2 text-xs text-slate-400">
            Se déconnecter
          </button>
        </form>
      </header>
      ...
      <div className="flex flex-col gap-5">
        ...
        {/* 2. Prochain départ — raccourcis d'abord (un tap), heure précise
            en repli. Voir actions.ts : les deux écritures sont calculées
            côté serveur, jamais avec l'horloge du téléphone. */}
        <section className="rounded-xl border border-white/10 bg-white/[0.03] p-5">
          ...
          <form action={definirProchainDepart} className="mt-3 flex gap-2">
            <input
              type="time"
              name="heure"
              required
              className="flex-1 rounded-lg border border-white/15 bg-transparent px-3 py-3 text-base text-white outline-none focus:border-cyan-400/60"
            />
            <button type="submit" className="shrink-0 rounded-lg border border-white/15 px-4 text-sm text-slate-300">
              Définir
            </button>
          </form>
        </section>
        ...
        {/* 3. Compteurs — mêmes chiffres que le dashboard, voir
            lireCompteursDuJour (lib/mission-nerf.ts). */}
        <section className="grid grid-cols-2 gap-3">
          <div className="rounded-xl border border-cyan-400/20 bg-white/[0.03] p-4">
            ...
            <p className="mt-1 text-3xl font-bold text-cyan-300">{compteurs.participants}</p>
          </div>
          <div className="rounded-xl border border-pink-400/20 bg-white/[0.03] p-4">
            <p className="text-[10px] uppercase tracking-wide text-slate-400">Décharges</p>
            <p className="mt-1 text-3xl font-bold text-pink-300">{compteurs.decharges}</p>
          </div>
        </section>
        ...
        <section className="rounded-xl border border-white/10 bg-white/[0.03] p-5">
          <p className="mb-3 text-xs uppercase tracking-wide text-slate-400">Dernières inscriptions</p>
          <ul className="flex flex-col gap-2">
            {dernieres.length === 0 ? (
              <li className="text-sm text-slate-500">Aucune inscription aujourd&apos;hui.</li>
            ) : (
              dernieres.map((d, i) => (
                <li
                  key={i}
                ...
                >
                  ...
                  <span className="shrink-0 text-slate-400">{heureQuebec(d.recu_le)}</span>
                </li>
              ))
            )}
          </ul>
